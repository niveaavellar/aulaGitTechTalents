...
//		System.out.println("A densidade populacional de " + nome + " é " + densidade);
	...
	public void comparaFronteiras(Pais pais) {
		ArrayList<String> fronteiraComum;
		fronteiraComum = new ArrayList<>();
		
		for (int i = 0; i < fronteira.size(); i++) {
			for (int cont = 0; cont < pais.fronteira.size(); cont++) {
				if(fronteira.get(i) == pais.fronteira.get(cont)) {
					fronteiraComum.add(pais.fronteira.get(cont).getNome());
				}
				
			}
			
		}
		System.out.println("Lista dos paises que fazem fronteira com " + nome + " e " + pais.getNome());
		for (int x = 0; x < fronteiraComum.size(); x++) {
			System.out.println(fronteiraComum.get(x));
		}
